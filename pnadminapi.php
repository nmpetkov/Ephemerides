 ...
 * @version $Id: pnadminapi.php 420 2010-06-14 04:56:34Z drak $
...
function ephemerids_adminapi_getlinks()
{
    ...
    if (SecurityUtil::checkPermission('Ephemerids::', '::', ACCESS_READ)) {
        $links[] = array('url' => ModUtil::url('Ephemerids', 'admin', 'view'), 'text' => __('Ephemerids list', $dom));
    }
    if (SecurityUtil::checkPermission('Ephemerids::', '::', ACCESS_ADD)) {
        $links[] = array('url' => ModUtil::url('Ephemerids', 'admin', 'new'), 'text' => __('Create new ephemerid', $dom));
    }
    if (SecurityUtil::checkPermission('Ephemerids::', '::', ACCESS_ADMIN)) {
        $links[] = array('url' => ModUtil::url('Ephemerids', 'admin', 'modifyconfig'), 'text' => __('Settings', $dom));
    }
